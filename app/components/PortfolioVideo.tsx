...
type Props = {
  ...
  /** Większy przycisk play — dla pojedynczego, centralnego wideo (np. VSL). */
  large?: boolean;
};

/**
 * Kadr wideo w języku "PLAN": uniesiony arkusz (surface-panel),
    ...
    >
      {playing ? (
        ...
        >
          ...
          <span
            aria-hidden="true"
            className={`absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center bg-accent text-accent-foreground shadow-[0_14px_30px_-12px_rgba(20,21,13,0.65)] transition-transform duration-300 ease-out group-hover:scale-110 ${
              large
                ? "h-20 w-20 lg:h-[92px] lg:w-[92px]"
